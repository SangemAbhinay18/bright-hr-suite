...
const EmployeeExpenses = () => {
  const [data, setData] = useState(initialExpenses.filter(e => e.employeeId === '1'));
  ...
  const handleSubmit = () => {
    ...
    setData([...data, { id: Date.now().toString(), employeeId: '1', name: 'You', ...form, date: new Date().toISOString().split('T')[0], status: 'pending' }]);
    setShowModal(false);
              ...
              {data.map(e => (
                <tr key={e.id} className="border-b last:border-0 hover:bg-accent/30">
                  ...
                  <td className="px-4 py-3">
                    <span className={`rounded-full px-2.5 py-0.5 text-xs font-medium ${e.status === 'approved' ? 'bg-success/10 text-success' : 'bg-warning/10 text-warning'}`}>{e.status}</span>
                  </td>
